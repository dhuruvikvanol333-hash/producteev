...
export class SpaceService {
  ...
  static async getByUser(userId: string, orgId?: string) {
    ...
    // If orgId is provided, we only check that specific organization
    ...
    for (const membership of memberships) {
      // Only OWNER and ADMIN see all spaces.
      // MEMBER, GUEST, LIMITED_MEMBER, etc. only see spaces/lists they are assigned to.
  ...
  static async getSpaceTasks(spaceId: string, filter: string, userId: string) {
    ...
    switch (filter) {
      ...
      case 'all':
        ...
        break;
      case 'starred':
        baseWhere.favoritedBy = { some: { userId } };
        break;
      default:
        break;
    }
