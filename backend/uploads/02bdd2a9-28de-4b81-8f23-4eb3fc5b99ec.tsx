...
export function DeleteConfirmModal({ type, title, id, onClose, onSuccess }: DeleteConfirmModalProps) {
  ...
  const handleDelete = async () => {
    try {
      ...
      const endpoint = type === 'Space' ? `/spaces/${id}` : type === 'Folder' ? `/folders/${id}` : `/lists/${id}`;
        ...
        <div className="p-6 text-center">
          ...
          <div className="flex flex-col gap-2">
            <button
              ...
              className="w-full py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl shadow-lg shadow-red-500/20 transition-all active:scale-95 disabled:opacity-50"
            >
