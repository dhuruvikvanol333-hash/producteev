...
export class SpaceController {
  ...
  getByUser = asyncHandler(async (req: Request, res: Response) => {
    if (!req.user) throw ApiError.unauthorized();
    const spaces = await SpaceService.getByUser(req.user.id);
    res.json({ success: true, data: spaces });
  });
  ...
  setUserGranularMemberships = asyncHandler(async (req: Request, res: Response) => {
    ...
    const memberships = await SpaceService.setUserGranularMemberships(organizationId as string, userId as string, data);
    res.json({ success: true, data: memberships });
  });
