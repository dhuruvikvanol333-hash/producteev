  ...
  return (
    ...
    >
      <div className="relative z-10">
        <span className="text-4xl mb-4 block" role="img" aria-label="greeting icon">{icon}</span>
...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    let timeout: any;
    const handleRefresh = () => {
      // Proper debounce to prevent multiple simultaneous refreshes
    ...
    socket.on('message:new', (msg: any) => {
      if (msg?.receiverId === currentUser?.id) {
        // Only increment if not currently viewing this chat (if we had a route check here too)
                      ...
                      <div className="flex items-center gap-4 min-w-0">
                        ...
                        {task.tags && task.tags.length > 0 && (
                          <div className="flex items-center gap-1.5 shrink-0 flex-wrap">
                            ...
                            {task.tags.length > 3 && (
                              <span className="text-[8px] font-black text-gray-400 dark:text-gray-500">+{task.tags.length - 3}</span>
                            )}
                          </div>
                        )}
                      </div>
