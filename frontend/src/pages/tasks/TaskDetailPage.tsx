...
const DISPLAY_STATUSES = [
  { key: 'OPEN', label: 'OPEN', color: '#52606D', dotStyle: 'solid', bg: '#F1F5F8', textColor: '#3E4C59', backendStatus: 'OPEN' as TaskStatus, section: 'open' },
  { key: 'PENDING', label: 'PENDING', color: '#243B53', dotStyle: 'solid', bg: '#FADB5E', textColor: '#243B53', backendStatus: 'PENDING' as TaskStatus, section: 'open' },
  { key: 'IN_PROGRESS', label: 'IN PROGRESS', color: '#ffffff', dotStyle: 'solid', bg: '#E11D48', textColor: '#ffffff', backendStatus: 'IN_PROGRESS' as TaskStatus, section: 'open' },
  { key: 'COMPLETED', label: 'COMPLETED', color: '#38BDF8', dotStyle: 'solid', bg: '#000000', textColor: '#ffffff', backendStatus: 'COMPLETED' as TaskStatus, section: 'open' },
  { key: 'IN_REVIEW', label: 'IN REVIEW', color: '#ffffff', dotStyle: 'solid', bg: '#EA580C', textColor: '#ffffff', backendStatus: 'IN_REVIEW' as TaskStatus, section: 'open' },
  { key: 'ACCEPTED', label: 'ACCEPTED', color: '#ffffff', dotStyle: 'solid', bg: '#C5221F', textColor: '#ffffff', backendStatus: 'ACCEPTED' as TaskStatus, section: 'open' },
  { key: 'REJECTED', label: 'REJECTED', color: '#ffffff', dotStyle: 'solid', bg: '#9C27B0', textColor: '#ffffff', backendStatus: 'REJECTED' as TaskStatus, section: 'open' },
  { key: 'CLOSED', label: 'CLOSED', color: '#ffffff', dotStyle: 'solid', bg: '#2E7D32', textColor: '#ffffff', backendStatus: 'CLOSED' as TaskStatus, section: 'closed' },
];
...
function StatusBadge({ ds, onClick, canUpdate }: { ds: any; onClick: () => void; canUpdate: boolean }) {
  const ref = useRef<HTMLButtonElement>(null);
  useEffect(() => {
    if (ref.current) {
      ref.current.style.color = ds.textColor;
      ref.current.style.backgroundColor = ds.bg;
      ref.current.style.boxShadow = `0 1px 2px ${ds.bg}40`;
      ref.current.style.borderColor = ds.textColor + '30';
    }
  }, [ds]);
  ...
  return (
    <button
      ref={ref}
      onClick={onClick}
      className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-black tracking-tight uppercase border transition-all ${!canUpdate ? 'opacity-70 cursor-not-allowed' : 'hover:brightness-95 shadow-sm active:scale-95'}`}
      disabled={!canUpdate}
      title="Change status"
    >
      {ds.label}
      {canUpdate && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" className="ml-1.5 opacity-60"><path d="M6 9l6 6 6-6" /></svg>}
    </button>
  );
}

function StatusOption({ s }: { s: any }) {
  const ref = useRef<HTMLSpanElement>(null);
  useEffect(() => {
    if (ref.current) {
      ref.current.style.backgroundColor = s.bg;
      ref.current.style.color = s.textColor;
      ref.current.style.borderColor = s.textColor + '30';
    }
  }, [s]);
...
export function TaskDetailPage({ isModal = false, taskId: propTaskId, onClose }: { isModal?: boolean, taskId?: string, onClose?: () => void }) {
  ...
  const canEditDescription = isOwner || (task && currentUser && task.createdById === currentUser.id);
  ...
  const [seenCommentCount, setSeenCommentCount] = useState<number>(0);
  ...
  const loadComments = async () => {
    ...
    try {
      ...
      setSeenCommentCount(res.data.data.length);
    } catch {
      // ignore
    }
  };
  ...
  const handlePostComment = async () => {
    if (!comment.trim() || !id || isPostingComment) return;
    try {
          ...
          <div className="max-w-[900px] mx-auto px-6 py-5">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-3 mb-6 text-sm">
                ...
                <div className="relative" ref={statusRef}>
                  ...
                  {statusDropdown && (
                    <div className="absolute left-0 top-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl dark:shadow-gray-900 z-50 w-[220px] overflow-hidden">
                      ...
                      {/* Open statuses */}
                      <div className="py-1">
                        <div className="px-3 py-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Statuses</div>
                        {DISPLAY_STATUSES.filter(s => s.section === 'open' && s.label.toLowerCase().includes(statusSearch.toLowerCase())).map(s => (
                          ...
                          >
                            ...
                            {displayStatusKey === s.key && (
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" className="text-indigo-600 dark:text-indigo-400">
                      ...
                      <div className="border-t border-gray-100 dark:border-gray-700 py-1">
                        <div className="px-3 py-1 text-[11px] font-black uppercase tracking-[0.1em] text-slate-500 dark:text-slate-400">Closed</div>
                  ...
                  {assigneeDropdown && (
                    <div className="absolute left-0 top-full mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl dark:shadow-gray-900 z-[100] w-[280px] overflow-hidden animate-scale-in origin-top-left">
                      ...
                      <div className="flex items-center justify-between p-2.5 pb-1">
                        <div className="flex items-center gap-1 flex-wrap">
                          {assignees.map(u => (
                            <div key={u.id} className="relative">
                              {u.avatarUrl ? (
                                <img src={u.avatarUrl} alt="" className="w-7 h-7 rounded-lg border border-white dark:border-gray-800 object-cover shadow-sm" />
                              ) : (
                                <div className="w-7 h-7 rounded-lg bg-indigo-500 text-white text-[9px] font-bold flex items-center justify-center border border-white dark:border-gray-800 shadow-sm">
                        ...
                        <button onClick={() => setAssigneeDropdown(false)} className="p-1 text-gray-400 hover:text-gray-600 transition-colors" title="Close dropdown">
                              ...
                              <div className="flex items-center gap-3">
                                <div className="relative shrink-0">
                                  <div className={cn(
                                    ...
                                    {u.avatarUrl ? (
                                      <img src={u.avatarUrl} alt="" className="w-8 h-8 rounded-lg object-cover border border-gray-100 dark:border-gray-800" />
                                    ) : (
                                      <div className={cn(
                ...
                <div className="flex items-center gap-1.5 text-xs">
                  {editingStartDate ? (
                    <input
                      ...
                      onChange={(e) => {
                        updateTask({ startDate: e.target.value || null });
                        setEditingStartDate(false);
                      }}
                      onBlur={() => setEditingStartDate(false)}
                      autoFocus
                      className="text-xs border border-gray-200 dark:border-gray-700 rounded-md px-1.5 py-0.5 outline-none bg-transparent dark:text-gray-300"
                      title="Start date"
                    />
                  ) : (
                    <button
              ...
              <div className="flex items-start gap-3 col-span-2">
                <div className="flex items-center gap-2 w-[130px] shrink-0 text-gray-500 dark:text-gray-400 pt-1.5">
                  <IconTrackTime />
            ...
            <div className="mb-8 border-t border-gray-100 dark:border-gray-700 pt-5">
              {editingDesc ? (
                ...
                >
                  {task.description ? (
                    <div
                      className="space-y-1 editor-content flex-1 py-1"
                      dangerouslySetInnerHTML={{ __html: linkifyHtmlText(showMore ? task.description : (task.description.length > 500 ? task.description.slice(0, 500) + '...' : task.description)) }}
                    />
                  ) : (
                    <div className="flex items-center gap-2.5 text-gray-400 dark:text-gray-500 flex-1 py-1 group">
                      ...
                      <span className="text-[13px] italic">Write or type '/' for commands and AI actions</span>
                    </div>
                  )}
                </div>
              )}
      ...
      <div className="w-[360px] shrink-0 flex flex-col bg-white dark:bg-gray-900 hidden lg:flex">
        ...
        <div className="flex items-center gap-1 px-3 py-2 border-b border-gray-100 dark:border-gray-700">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium transition-colors ${activityTab === 'activity'
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium transition-colors ${activityTab === 'comments'
              ? 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300'
              : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'
              }`}
            title="Comments Feed"
          >
            ...
            Comments
            {(comments.length - seenCommentCount) > 0 && (
              <span className="ml-0.5 px-1.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-[10px]">
                {comments.length - seenCommentCount}
              </span>
            )}
          </button>
        </div>
        ...
        {canAddComments ? (
          <div className="border-t border-gray-200 dark:border-gray-700 p-3 mt-auto">
            <div className="flex items-end rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 pr-2">
              <textarea
                value={comment}
                ...
                className="flex-1 px-3 py-2.5 bg-transparent text-sm text-gray-700 dark:text-gray-300 placeholder-gray-400 dark:placeholder-gray-500 outline-none resize-none disabled:opacity-50"
                rows={1}
