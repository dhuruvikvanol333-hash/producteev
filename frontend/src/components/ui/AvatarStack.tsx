...
export function AvatarStack({ 
  ...
  if (users.length === 0 && showPlaceholder) {
    return (
      <div className={`${sizeClasses[size]} rounded-lg border-2 border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-center text-gray-300 ${className}`}>
  ...
  return (
    <div className={`flex -space-x-1.5 hover:space-x-0.5 transition-all duration-300 group/stack ${className}`}>
      {visibleUsers.map((user) => (
        ...
        >
          <div
            className={`${sizeClasses[size]} rounded-lg border-2 border-white dark:border-gray-900 bg-gray-100 dark:bg-gray-800 flex items-center justify-center overflow-hidden shadow-sm transition-transform hover:scale-110 hover:z-10`}
              ...
              <img 
                ...
                onError={(e) => {
                  ...
                  if (parent && !parent.querySelector('.avatar-placeholder')) {
                    ...
                    span.className = 'avatar-placeholder font-black uppercase tracking-tighter text-gray-600 dark:text-gray-300';
                    span.innerText = `${user.firstName ? user.firstName[0] : '?'}${user.lastName ? user.lastName[0] : ''}`;
      ...
      {remainingCount > 0 && (
        <div
          className={`${sizeClasses[size]} rounded-lg border-2 border-white dark:border-gray-900 bg-gray-200 dark:bg-gray-700 flex items-center justify-center shrink-0 shadow-sm`}
        >
